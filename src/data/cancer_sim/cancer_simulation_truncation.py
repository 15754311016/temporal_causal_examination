...
with open('/home/qianghuang/CausalTransformer-main/config/config.yaml', 'r') as file:
    canser_para_data = yaml.load(file, Loader=yaml.FullLoader)
    chemo_constant = canser_para_data['exp']['chemo']
    radio_constant = canser_para_data['exp']['radio']
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def simulate_factual(simulation_params, seq_length, assigned_actions=None):
    ...
    radio_amt = np.array([radio_constant for i in range(total_num_radio_treatments)])  # Gy
    # radio_days = np.array([i + 1 for i in range(total_num_radio_treatments)])
    chemo_amt = [chemo_constant for i in range(total_num_chemo_treatments)]
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(1, seq_length - 1):
            ...
            ####
            # if i==10:
            #     print("cancer_volume[i, t]:", cancer_volume[i, t])
            #     print("current_chemo_dose:",current_chemo_dose)
            #     print("radio_dosage:",current_chemo_dose)
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(0, seq_length - 1):
            ...
            for treatment_option in treatment_options:
                ...
                truncation_size = 4
                # if t==10:
                #     temp = np.append(factual_cancer_volume[:t + 1],[counterfactual_cancer_volume])
                #     new_temp = np.append(temp[t-truncation_size:],np.zeros(t-truncation_size))
                #     print("length and cancer volume:", len(cancer_volume[test_idx][:t + 2]), cancer_volume[test_idx][:t + 2])
                #     print("length and new:", len(new_temp), new_temp)
                #     print(sd)
                if t <=truncation_size:
                    ...
                    cancer_volume[test_idx][:t + 2] = np.append(np.append(factual_cancer_volume[:t + 1],[counterfactual_cancer_volume])[t-truncation_size:]
                                                                ,np.zeros(t-truncation_size))
                    chemo_application_point[test_idx][:t + 1] = np.append(np.append(factual_chemo_application_point[:t],[counterfactual_chemo_application_point])[t-truncation_size:]
                                                                          ,np.zeros(t-truncation_size))
                    radio_application_point[test_idx][:t + 1] = np.append(np.append(factual_radio_application_point[:t],[counterfactual_radio_application_point])[t-truncation_size:]
                                                                          ,np.zeros(t-truncation_size))
                    sequence_lengths[test_idx] = int(t) + 1 - (int(t)-truncation_size)
                patient_types_all_trajectories[test_idx] = patient_types[i]
                test_idx = test_idx + 1
...
def simulate_counterfactuals_treatment_seq(simulation_params, seq_length, projection_horizon, cf_seq_mode='sliding_treatment'):
    ...
    test_idx = 0
    # data_save = {}
    # data_save.update({'num_patients':num_patients, 'num_patients': num_patients, 'num_test_points':num_test_points, 'seq_length':seq_length, 'horizon':projection_horizon})
    # np.savez('multi_statics.npz', **data_save)
    # Run actual simulation
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(0, seq_length - 1):
            ...
            for treatment_option in treatment_options:
                ...
                if t<=truncation_size:
                    ...
                    cancer_volume[test_idx][:t + 1 + projection_horizon + 1] = np.append(counterfactual_cancer_volume[t-truncation_size:], np.zeros(t-truncation_size))
                    chemo_application_point[test_idx][:t + 1 + projection_horizon] = np.append(counterfactual_chemo_application_point[t-truncation_size:], np.zeros(t-truncation_size))
                    radio_application_point[test_idx][:t + 1 + projection_horizon] = np.append(counterfactual_radio_application_point[t-truncation_size:],np.zeros(t-truncation_size))
                    sequence_lengths[test_idx] = int(t) + projection_horizon + 1 - (int(t)-truncation_size)
                patient_types_all_trajectories[test_idx] = patient_types[i]
