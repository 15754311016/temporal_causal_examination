...
class SyntheticCancerDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.params = generate_params(num_patients, chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size,
                                      lag=lag, binary_treatments=binary_treatments)
...
class SyntheticCancerDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        self.train_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag, treatment_mode=treatment_mode,binary_treatments=binary_treatments)
        self.val_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['val'], window_size, max_seq_length, 'val',
                                            lag=lag, treatment_mode=treatment_mode,binary_treatments=binary_treatments)
        self.test_cf_one_step = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                       ...
                                                       treatment_mode=treatment_mode,binary_treatments=binary_treatments)
        self.test_cf_treatment_seq = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                            ...
                                                            cf_seq_mode=cf_seq_mode, treatment_mode=treatment_mode,binary_treatments=binary_treatments)
        # np.savez('test_cf_one_step.npz', **self.test_cf_one_step.data)
        # np.savez_compressed(os.path.join(os.getcwd(), 'test_cf_one_step.npz'), **self.test_cf_one_step.data)
        # np.savez_compressed(os.path.join(os.getcwd(), 'test_cf_treatment_seq.npz'), **self.test_cf_treatment_seq.data)
        # np.savez('test_cf_treatment_seq.npz', **self.test_cf_treatment_seq.data)
        self.projection_horizon = projection_horizon
        self.autoregressive = True
        self.has_vitals = False
        self.train_scaling_params = self.train_f.get_scaling_params()
        with open(os.path.join(os.getcwd(), 'train_scaling_params.pt'), 'wb') as file:
            pickle.dump(self.train_scaling_params, file)
        # np.savez_compressed(os.path.join(os.getcwd(), 'train_scaling_params.npz'), **self.train_scaling_params)
