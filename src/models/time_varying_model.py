...
class TimeVaryingCausalModel(LightningModule):
    ...
    def get_normalised_masked_rmse(self, dataset: Dataset, one_step_counterfactual=False):
        ...
        if unscale:
            ...
            # np.savez_compressed(os.path.join(os.getcwd(), 'mean_std.npz'), np.array([output_stds, output_means]))
            ...
            if dataset.subset_name == 'test':
                data_to_save.update({
                    'means': outputs_unscaled, 'output': dataset.data['unscaled_outputs'], 'active_entries': dataset.data['active_entries'],
                    'treatments':dataset.data['current_treatments']
                })
                np.savez_compressed(os.path.join(os.getcwd(), 'predictions_encoder.npz'), **data_to_save)
        else:
            # Batch-wise masked-MSE calculation is tricky, thus calculating for full dataset at once
    ...
    def get_normalised_n_step_rmses(self, dataset: Dataset, datasets_mc: List[Dataset] = None):
        ...
        if unscale:
            ...
            if dataset.subset_name == 'test':
                data_to_save.update({
                    'means': outputs_unscaled, 'output': dataset.data_processed_seq['unscaled_outputs'], 'active_entries': dataset.data_processed_seq['active_entries'],
                    'treatments':dataset.data_processed_seq['current_treatments']
                })
                np.savez_compressed(os.path.join(os.getcwd(), 'predictions_multi_steps.npz'), **data_to_save)
...
class BRCausalModel(TimeVaryingCausalModel):
    ...
    def training_step(self, batch, batch_ind, optimizer_idx=0):
        ...
        if optimizer_idx == 0:  # grad reversal or domain confusion representation update
            ...
            loss =  self.hparams.exp.weight_bceloss * bce_loss + mse_loss
